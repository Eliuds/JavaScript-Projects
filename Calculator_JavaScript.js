// creates an object to keep tracks values
const Calculator = {
    //This displays 0 on the screen
    Display_Value: '0',
    //this will hold the first operand for any exxpressions, we set it to null for now.
    First_Operand: null,
    // this checks whether or not the seconds operand has been input
    Wait_Seconds_Operand: false,
    //this will hold the operator, we set it to null for now
    operator: null
};

//This modifies values each time a buttos is clicked
function Input_Digit(digit) {
    const { Display_Value, Wait_Seconds_Operand} = Calculator;
    //we are checking to see if waitsecoperand is true and set
...
function Handle_Operator(Next_Operator) {
    ...
    //this will removed any trailing 0's
...
keys.addEventListener('click', (event) => {
    ...
    // ensures that AC clears the numbers fron the Calculator
